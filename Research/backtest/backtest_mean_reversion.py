...
def backtrade_with_signals(minute_df):
    """Perform backtrading using the signal column in the DataFrame and compute backtesting metrics."""
    if 'signal' not in minute_df.columns:
    ...
    win_rate = (wins / trades) * 100 if trades > 0 else 0  # Corrected win rate calculation
    ...
    # Plot Trading Signal
    axs[1].plot(minute_df.index, minute_df['signal'].apply(lambda x: 1 if x == 'buy' else -1 if x == 'sell' else 0), label='Signal')
    axs[1].set_title('Trading Signal')
    axs[1].legend()
    ...
    axs[0].plot(minute_df.index, [-d for d in drawdown_history], label='Drawdown')
    axs[0].set_title('Drawdown Over Time')
    axs[0].legend()

    # Plot Sharpe Ratio Over Time
    axs[1].plot(minute_df.index[2:], sharpe_history, label='Sharpe Ratio')
    axs[1].set_title('Sharpe Ratio Over Time')
    axs[1].legend()
...
if ticker_minute_data is not None:
    ...
    backtrade_with_signals(ticker_minute_data)
